...
if os.path.exists(DB_PATH):
    ...
    if not view_data.empty:
        ...
        # --- AI COACH SECTION ---
        st.write("---")
        st.subheader("💬 Ask Coach Arnold (Science-Backed)")

        user_question = st.text_input("Ask about your progress or general training science:")

        if st.button("Ask Coach"):
            if user_question:
                with st.spinner("Searching..."):
                    db = load_science_db()  # This will be nearly instant after the first run
                    ...
                    try:
                        stream = ollama.chat(
                            ...
                            messages=[
                                {
                                    "role": "system",
                                    "content": "You are Coach Arnold. Use the provided Science and History to give elite advice. Cite the 'Manual' if you use science."
                                },
                                {
                                    "role": "user",
